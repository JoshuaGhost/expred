...
def load_dataset(docs_folder, dataset_fname):
    ...
    for a in d:
        ...
        classification = 1 if annotation['classification'] == 'True' else 0
        doc_fname = os.path.join(
            docs_folder, annotation['annotation_id'].split(':')[0])
        try:
...
def download_and_load_datasets(force_download=False):
    dataset = tf.keras.utils.get_file(
        fname="multirc.tar.gz",
        origin="http://www.eraserbenchmark.com/zipped/multirc.tar.gz",
        extract=True)
    dataset_folder = os.path.join(os.path.dirname(dataset), 'multirc')
