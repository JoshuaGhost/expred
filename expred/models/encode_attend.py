...
def initialize_model(params: dict, vocab: Set[str], batch_first: bool, unk_token='UNK'):
    ...
    if 'embedding_file' in params['embeddings']:
        ...
        raise ValueError("No 'embedding_file' found in conf!")
...
def main():
    ...
    with open(os.path.join(args.output_dir, 'test_decoded.jsonl'), 'w') as test_output:
        for line in test_rats:
            ...
            test_output.write('\n')
    #training_results, train_decoded, val_decoded, test_decoded = decode(classifier, interned_train, interned_val, interned_test, interned_documents, evidence_classes, conf['classifier']['batch_size'], tensorize_model_inputs=True)
